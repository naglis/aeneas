...
Python C Extension for synthesizing text with eSpeak NG
...
int _initialize_synthesizer(char const *output_file_path) {
	...
	*/
	...
	// open wave file
    if (wave_file == NULL) {
        if(_open_wave_file(output_file_path, sample_rate) != CENGW_SUCCESS) {
            return CENGW_FAILURE;
        }
	}
...
int _synthesize_multiple(
    ...
    if ((backwards != 0) && (quit_after > 0)) {
        ...
        if (_initialize_synthesizer(output_file_path) != CENGW_SUCCESS) {
            return CENGW_FAILURE;
        }

        // synthesize from the back
        for (i = number_of_fragments - 1; ; --i) {
            if (_set_voice_code((*fragments_ret)[i].voice_code) != CENGW_SUCCESS) {
                return CENGW_FAILURE;
            }
            if (_synthesize_string((*fragments_ret)[i].text) != CENGW_SUCCESS) {
                return CENGW_FAILURE;
            }
            start = i;
