...
static PyObject *synthesize_multiple(PyObject *self, PyObject *args) {
    ...
    int sample_rate; // int because espeak-ng lib returns it as such
    ...
    // NOTE: returning sample_rate as an int, as the espeak-ng lib does
    tuple = PyTuple_New(3);
...
static PyMethodDef cengw_methods[] = {
    {
        "synthesize_multiple",
        synthesize_multiple,
        METH_VARARGS,
        "Synthesize multiple text fragments with eSpeak NG\n"
...
PyMODINIT_FUNC PyInit_cengw(void) {
    return moduleinit();
}
